...
void Controller::createProject ()
{
	...
	auto pluginUseStandalone = model->getValue (valueIdStandalone)->getValue () != 0;
	        ...
	        CommonDirectoryLocation::AppResourcesPath))
	{
		*scriptPath += "GenerateVST3Plugin.cmake";
		...
		if(auto pluginUseStandalone = model->getValue (valueIdStandalone)->getValue () != 0)
		{
			args.add("-DSMTG_ENABLE_STANDALONE_CLI=ON");
		}
		else
		{
			args.add("-DSMTG_ENABLE_STANDALONE_CLI=OFF");
		}
...
void Controller::runProjectCMake (const std::string& path)
{
	...
	if (auto process = Process::create (cmakePathStr.getString ()))
	{
		...
		if(auto pluginUseStandalone = model->getValue (valueIdStandalone)->getValue () != 0)
		{
			args.add("-DSMTG_ENABLE_STANDALONE=ON");
		}
		else
		{
			args.add("-DSMTG_ENABLE_STANDALONE=OFF");
		}
